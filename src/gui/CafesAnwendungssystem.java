...
public class CafesAnwendungssystem {
    ...
    // speichert temporaer ein Objekt vom Typ cafe
    private Cafe cafe;
    
    public CafesAnwendungssystem(Stage primaryStage){
    	...
    	try{ 
    		this.cafe = new Cafe(
    			txtName.getText(), 
   	        	...
   	        	txtMitBaeckerei.getText(),
    		    txtKaffeeprodukte.getText().split(";"));
    		zeigeInformationsfensterAn("Das Cafe wurde aufgenommen!");
       	}
       	catch(Exception exc){
   ...
   public void zeigeCafeAn(){
    ...
    public void schreibeCafesInCsvDatei() {
		try {
			BufferedWriter aus 
				= new BufferedWriter(new FileWriter("cafesAusgabe.csv", true));
			aus.write(this.cafe.gibCafeZurueck(';'));
			aus.close();
   			zeigeInformationsfensterAn(
	   			"Die Cafes wurden gespeichert!");
		}	
		catch(IOException exc){
			zeigeFehlermeldungsfensterAn(
